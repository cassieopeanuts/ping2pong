...
pub fn create_player(player: Player) -> ExternResult<Record> {
    ...
    if !is_player_name_unique(&player.player_name)? {
     ...
     let existing_links = get_links(
        LinkQuery::try_new(player.player_key.clone(), LinkTypes::PlayerToPlayers)?,
        GetStrategy::default(),
     )?;
...
pub fn get_latest_player(original_player_hash: ActionHash) -> ExternResult<Option<Record>> {
    let links = get_links( LinkQuery::try_new(original_player_hash.clone(), LinkTypes::PlayerUpdates)?, GetStrategy::default() )?;
...
pub fn delete_player(original_player_hash: ActionHash) -> ExternResult<ActionHash> {
    ...
    // Delete links
    let links_agent = get_links( LinkQuery::try_new(player.player_key.clone(), LinkTypes::PlayerToPlayers)?, GetStrategy::default() )?;
    for link in links_agent { if let Some(action_hash) = link.target.into_action_hash() { if action_hash == original_player_hash { delete_link(link.create_link_hash, GetOptions::default())?; } } }
    let name_anchor = anchor_for(&player.player_name.to_lowercase())?;
    let links_name = get_links( LinkQuery::try_new(name_anchor, LinkTypes::PlayerNameToPlayer)?, GetStrategy::default() )?;
     for link in links_name { if let Some(action_hash) = link.target.into_action_hash() { if action_hash == original_player_hash { delete_link(link.create_link_hash, GetOptions::default())?; } } }
...
pub fn get_player_profile_hash_for_agent(player_agent_key: AgentPubKey) -> ExternResult<Vec<Link>> {
    get_links(LinkQuery::try_new(player_agent_key, LinkTypes::PlayerToPlayers)?, GetStrategy::default())
}
...
pub fn get_deleted_player_links_for_agent( player_agent_key: AgentPubKey, ) -> ExternResult<Vec<(SignedActionHashed, Vec<SignedActionHashed>)>> {
    let details = get_links_details( LinkQuery::try_new(player_agent_key, LinkTypes::PlayerToPlayers)?, GetStrategy::default() )?;
...
pub fn get_all_player_pubkeys(_: ()) -> ExternResult<Vec<AgentPubKey>> {
    ...
    let links = get_links(
        LinkQuery::try_new(all_players_anchor_hash.clone(), LinkTypes::AllPlayersAnchorToAgentPubKey)?,
        GetStrategy::default(),
    )?;
...
pub fn get_player_profile_by_agent_key(agent_key: AgentPubKey) -> ExternResult<Option<Record>> {
    let links = get_links(
        LinkQuery::try_new(agent_key.clone(), LinkTypes::PlayerToPlayers)?,
        GetStrategy::default(),
    )?;
